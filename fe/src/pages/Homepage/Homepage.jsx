...
import { Button, Grid, Typography, CardContent, Box, IconButton } from '@mui/material';
...
export default function Home() {
  ...
  useEffect(() => {
    if (videoLoaded) setIsLoading(false);
  }, [videoLoaded]);

  // Fetch blindbox data and sort for Trending and New Launch
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchBlindboxData();
        if (Array.isArray(data)) {
          // Sort for Trending (highest rating)
          const trending = [...data]
            .sort((a, b) => (b.rating || 0) - (a.rating || 0))
            .slice(0, 8);
          setTrendingProducts(trending);

          // Sort for New Launch (most recent)
          const newLaunches = [...data]
            .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
            .slice(0, 8);
          setNewLaunchProducts(newLaunches);
        }
      } catch (error) {
  ...
  const LazyImage = ({ src, alt, ...props }) => (
    <img src={src} alt={alt} width={90} height={90} loading="lazy" {...props} />
  );

  // Handle navigation to product detail page with id as query param
  const handleProductClick = (product) => {
    navigate(`/product/${product.slug}?id=${product._id}`);
  };
      ...
      <Grid container component="main">
        ...
        <Grid container component="main" sx={{ mt: 4 }}>
          ...
          <Grid item xs={5} data-aos="fade-up">
            <Typography fontFamily="'Jersey 15', sans-serif" variant="h3" sx={{ mb: 4, ...yellowGlowAnimation }}>
              Discover the Thrill of Mystery <br /> Buy & Sell Blind Boxes!
            </Typography>
        ...
        <Grid item xs={12} data-aos="fade-up">
          ...
          <Typography variant="subtitle1" sx={{ mt: 2, mb: 3, mx: 20, textAlign: "center", color: "white" }} data-aos="fade-up" data-aos-delay="200">
            Explore Our Exclusive Blind Box Collection: Discover limited-edition and rare collectibles from top brands like Pop Mart, Tokidoki, BE@RBRICK, and more. Each blind box is offered by trusted sellers at great prices, bringing you the thrill of surprise and exceptional value. Start your collection today!
          </Typography>
          <Stack direction="row" spacing={10} justifyContent="center" sx={{ mb: 3 }} data-aos="fade-up" data-aos-delay="400">
            <Avatar sx={{ width: 90, height: 90 }}>
              <LazyImage src="https://i.ebayimg.com/images/g/6bsAAOSwFlVhJpnu/s-l400.jpg" alt="Avatar 1" />
            </Avatar>
            <Avatar sx={{ width: 90, height: 90 }}>
              <LazyImage src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQHT7EIqHbuwCvGxYZpWf6M43MNxDxp3OpARg&s" alt="Avatar 2" />
            </Avatar>
            <Avatar sx={{ width: 90, height: 90 }}>
              <LazyImage src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSxkuZjIic131BsGapQEt-YWOKUX3WodyFr_A&s" alt="Avatar 3" />
            </Avatar>
            <Avatar sx={{ width: 90, height: 90 }}>
              <LazyImage src="https://www.tokidoki.it/cdn/shop/files/tokidoki_logo_pumpkin_carving_thumbnail.jpg?v=1634074134" alt="Avatar 4" />
            </Avatar>
            <Avatar sx={{ width: 90, height: 90 }}>
              <LazyImage src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRdv_Xuqk4HLZ0IJVmMJk1yUxqmtMssH0XNcA&s" alt="Avatar 5" />
            </Avatar>
            <Avatar sx={{ width: 90, height: 90 }}>
              <LazyImage src="https://firmatodesign.com/cdn/shop/collections/MMC-KAWS_1200x1200_png.webp?v=1677510790" alt="Avatar 6" />
            </Avatar>
          </Stack>
        ...
        <Grid item xs={12} data-aos="fade-up">
          ...
          <Box sx={{ width: '90%', position: 'relative', mx: 'auto', mt: 3 }}>
                    ...
                    >
                      ...
                      <CardContent>
                        <Typography variant="h6" sx={{ color: 'white' }}>
                          {product.name.length > 29 ? `${product.name.substring(0, 30)}...` : product.name}
                        </Typography>
            ...
            <IconButton className="swiper-button-prev-trending" sx={{ position: 'absolute', top: '50%', transform: 'translateY(-50%)', zIndex: 100, left: 0 }}>
              <Button><ArrowCircleLeftOutlinedIcon sx={{ color: 'white', fontSize: '30px' }} /></Button>
            </IconButton>
            <IconButton className="swiper-button-next-trending" sx={{ position: 'absolute', top: '50%', transform: 'translateY(-50%)', zIndex: 100, right: 0 }}>
              <Button><ArrowCircleRightOutlinedIcon sx={{ color: 'white', fontSize: '30px' }} /></Button>
            </IconButton>
          </Box>
